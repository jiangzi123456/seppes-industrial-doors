...
export default function ProjectSupportPage() {
  return <div className="project-support-page">
    ...
    <section className="ps-section ps-library" id="project-library"><div className="container">
      ...
      <div className="ps-library-grid">{library.map((item, index) => <article key={item.title}><div className="ps-library-card__image"><Image src={item.image} alt={item.title} fill sizes="(max-width: 760px) 100vw, 33vw" /><span>0{index + 1}</span></div><div className="ps-library-card__body"><div><span>{item.sector}</span><span>{item.region}</span></div><h3>{item.title}</h3><p>Project planning included door selection, safety strategy, technical confirmation and export-ready documentation.</p><Link href="/contact">Request a similar solution <ArrowRight size={16} /></Link></div></article>)}</div>
    </div></section>
