...
const posts = [
  ...
  {"slug":"cleanroom-door-solutions-food-pharmaceutical","category":"Solutions","date":"Sep 20, 2026","title":"Clean Access Solutions for Food and Pharmaceutical Facilities","excerpt":"Specify clean-area doors around pressure, cleaning methods, traffic and interlocks. A practical selection and validation guide for project teams.","read":"8 min","image":"/images/blog/cleanroom-airlock-concept.webp","tone":"green","aiImage":true},
  {"slug":"industrial-door-buyer-checklist-12-questions","category":"Buying Guides","date":"Sep 20, 2026","title":"12 Questions to Ask Before Ordering an Industrial Door","excerpt":"A buyer's checklist for duty, site conditions, safety, materials, controls and support—with useful answers and the gaps worth questioning.","read":"8 min","image":"/images/blog/industrial-door-planning-concept.webp","tone":"navy","aiImage":true},
  {"slug":"industrial-door-pre-delivery-inspection","category":"Product News","date":"Sep 20, 2026","title":"Inside the Factory: Industrial Door Testing Before Delivery","excerpt":"Separate design validation, equipment checks and site acceptance. A practical guide to inspection scope, safety functions and handover records.","read":"8 min","image":"/images/blog/industrial-door-inspection-concept.webp","tone":"purple","aiImage":true},
];
...
export function GeoNewsFeed() {
  ...
  return <div className="geo-blog">
    ...
    <section className="geo-latest" id="latest"><div className="container"><div className="geo-section-label"><b>Latest intelligence</b></div>
      ...
      {visible.length ? <div className="geo-card-grid">{visible.map((post) => <article className="geo-card" key={post.title}><div className={`geo-card__visual tone-${post.tone}`}><Image src={post.image} alt={`${post.title} — ${post.category}`} fill sizes="(min-width: 1100px) 33vw, (min-width: 650px) 50vw, 100vw" /><span>0{posts.indexOf(post) + 1}</span>{("aiImage" in post && post.aiImage) && <small className="geo-card__ai">AI illustration</small>}<b>{post.category}</b></div>
        <div className="geo-card__body"><p className="geo-meta">{post.date} <span>·</span> <Clock3 size={13} /> {post.read}</p><h3>{post.title}</h3><p>{post.excerpt}</p><Link href={`/source/blog/${post.slug}`} aria-label={`Read ${post.title}`}>Read insight <ArrowRight size={16} /></Link></div></article>)}</div> : <div className="geo-empty"><h3>No briefings found.</h3><p>Try another topic or clear your search.</p></div>}
    </div></section>
