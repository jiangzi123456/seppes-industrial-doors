...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    keywords: post.slug === "control-dust-and-airflow-between-production-zones"
...
export default async function BlogArticlePage({ params }: Props) {
  ...
  const hasGeoAnswer = isRapidDoorArticle || isAirflowArticle;
  ...
  const quickFacts = isAirflowArticle ? airflowFacts : rapidDoorFacts;
  const faqs = isAirflowArticle ? airflowFaqs : rapidDoorFaqs;
  const quickQuestion = isAirflowArticle
  ...
  const quickAnswer = isAirflowArticle
  ...
  const schema = {
    ...
    dateModified: isAirflowArticle ? "2026-09-09" : isRapidDoorArticle ? "2026-09-07" : post.dateISO,
  ...
  return <div className="blog-post">
    ...
    <article className="container blog-post__layout">
      ...
      <div className="blog-post__body">
        ...
        {hasGeoAnswer && <><section className="blog-post__answer" aria-labelledby="quick-answer-title"><p>Quick answer</p><h2 id="quick-answer-title">{quickQuestion}</h2><strong>{quickAnswer}</strong></section><dl className="blog-post__facts" aria-label={isAirflowArticle ? "Environmental separation decision points" : "Product reference data"}>{quickFacts.map(([label, value]) => <div key={label}><dt><Check />{label}</dt><dd>{value}</dd></div>)}</dl></>}
        {post.sections.map((section, index) => <section id={`section-${index + 1}`} key={section.heading}>
          ...
          {section.subsections?.map((subsection) => <div className="blog-post__subsection" key={subsection.heading}><h3>{subsection.heading}</h3>{subsection.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>)}
        ...
        {hasGeoAnswer && <section className="blog-post__faq" id="frequently-asked-questions"><span>FAQ</span><h2>{isAirflowArticle ? "Dust and Airflow Control Questions" : "High-Speed Roll-Up Door Questions"}</h2><div>{faqs.map(([question, answer]) => <details key={question}><summary>{question}</summary><p>{answer}</p></details>)}</div></section>}
