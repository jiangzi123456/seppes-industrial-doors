...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    keywords: post.keywords ?? (post.slug === "high-speed-roll-up-door-food-material-access"
...
export default async function BlogArticlePage({ params }: Props) {
  ...
  const hasGeoAnswer = Boolean(post.quickAnswer) || isRapidDoorArticle || isAirflowArticle || isFoodArticle;
  ...
  const quickFacts = post.quickFacts ?? (isFoodArticle ? foodFacts : isAirflowArticle ? airflowFacts : rapidDoorFacts);
  const faqs = post.faqs ?? (isFoodArticle ? foodFaqs : isAirflowArticle ? airflowFaqs : rapidDoorFaqs);
  const quickQuestion = post.quickQuestion ?? (isFoodArticle
  ...
  const quickAnswer = post.quickAnswer ?? (isFoodArticle
  ...
  return <div className={post.quickAnswer ? "blog-post blog-post--editorial" : "blog-post"}>
    ...
    <article className="container blog-post__layout">
      ...
      <div className="blog-post__body">
        ...
        {hasGeoAnswer && <><section className="blog-post__answer" aria-labelledby="quick-answer-title"><p>Quick answer</p><h2 id="quick-answer-title">{quickQuestion}</h2><strong>{quickAnswer}</strong></section><dl className="blog-post__facts" aria-label={post.quickFacts ? "Project decision points" : isFoodArticle ? "Food material entrance decision points" : isAirflowArticle ? "Environmental separation decision points" : "Product reference data"}>{quickFacts.map(([label, value]) => <div key={label}><dt><Check />{label}</dt><dd>{value}</dd></div>)}</dl></>}
        ...
        {post.sections.map((section, index) => <section id={`section-${index + 1}`} key={section.heading}>
          ...
          {section.review && <div className="blog-post__review"><div><h3>A useful answer</h3><p>{section.review.good}</p></div><div><h3>Follow up when</h3><p>{section.review.flag}</p></div></div>}
          {section.table && <div className="blog-post__table-wrap" role="region" aria-label={section.table.caption} tabIndex={0}><table><caption>{section.table.caption}</caption><thead><tr>{section.table.headers.map(header => <th scope="col" key={header}>{header}</th>)}</tr></thead><tbody>{section.table.rows.map(row => <tr key={row[0]}>{row.map((cell, cellIndex) => cellIndex === 0 ? <th scope="row" key={cellIndex}>{cell}</th> : <td key={cellIndex}>{cell}</td>)}</tr>)}</tbody></table></div>}
        ...
        {hasGeoAnswer && <section className="blog-post__faq" id="frequently-asked-questions"><span>FAQ</span><h2>{post.faqTitle ?? (isFoodArticle ? "Food Material Access Questions" : isAirflowArticle ? "Dust and Airflow Control Questions" : "High-Speed Roll-Up Door Questions")}</h2><div>{faqs.map(([question, answer]) => <details key={question}><summary>{question}</summary><p>{answer}</p></details>)}</div></section>}
        {post.relatedLinks && <><nav className="blog-post__reading" aria-label="Related articles"><h2>Keep exploring</h2>{post.relatedLinks.map(link => <Link href={link.href} key={link.href}>{link.label}<ArrowRight size={18} /></Link>)}</nav><section className="blog-post__project-cta"><p>FROM REQUIREMENTS TO CONFIGURATION</p><h2>Bring your project into focus.</h2><p>Share the opening dimensions, quantity, application, traffic, environment, controls, location and available drawings.</p><Link className="reference-button reference-button--red" href="/contact">Discuss your requirements <ArrowRight size={18} /></Link></section></>}
